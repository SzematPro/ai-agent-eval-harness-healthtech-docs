...
| MEASURE 1.2: AI system performance is evaluated | Deterministic CI gate (a key-free deterministic stub client, 315 cases); nightly live-model run; Promptfoo red-team | Deterministic gate is reproducible; live-model metrics are not frozen in the model card (reported in the eval reports) |
| MEASURE 1.3: Evaluation data is representative | 315 cases across 3 locales (en, es-419, pt-BR); 5 condition clusters; golden + adversarial + no-match categories | Small sample; no demographic stratification (no demographic data collected); US-English bias acknowledged |
